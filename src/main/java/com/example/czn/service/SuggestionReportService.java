...
//    private MessageDao messageDao = DaoFactory.getFactory().getMessageDao();
    ...
    private SuggestionRepo suggestionRepo = TelegramBorRepositoryProvider.getSuggestionRepo();
    ...
    private void sendSuggesReport(long chatId, DefaultAbsSender bot, Date dateBegin, Date dateEnd, int messagePrevReport) throws TelegramApiException, IOException {
        ...
        List<Suggestion> reports = suggestionRepo.findAllByPostDateBetween(dateBegin, dateEnd);
        ...
        //  ------------------------------------------------------------- //
        int rowIndex = 0;
        Message message = messageRepo.findByIdAndLangId(Const.TABLE_HEAD_NAME, LanguageService.getLanguage(chatId).getId());
        String[] t = message.getName().split(Const.SPLIT);
        List<String> messageNames = new ArrayList<>(Arrays.asList(t));
        createTitle(styleTitle, rowIndex, messageNames);
        //createTitle(styleTitle, rowIndex, Arrays.asList(messageRepo.findMessageByIdAndLanguage(Const.TABLE_HEAD_NAME, LanguageService.getLanguage(chatId)).split(Const.SPLIT)));
