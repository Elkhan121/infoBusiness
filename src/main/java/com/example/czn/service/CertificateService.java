...
import com.example.czn.dao.repositories.CertificateRepo;
...
public class CertificateService {
    ...
    private CertificateRepo certificateRepo = TelegramBorRepositoryProvider.getCertificateRepo();
    ...
    public void sendCertificateReport(long chatId, DefaultAbsSender bot, Date dateBegin, Date dateEnd, int messagePrevReport) {
        currantLanguage = LanguageService.getLanguage(chatId);
        try {
            sendCertReport(chatId, bot, dateBegin, dateEnd, messagePrevReport);
        } catch (Exception e) {
    ...
    private void sendCertReport(long chatId, DefaultAbsSender bot, Date dateBegin, Date dateEnd, int messagePrevReport) throws TelegramApiException, IOException {
        sheets = workbook.createSheet(getText(Const.CERTIFICATE));
        sheet = workbook.getSheetAt(0);
        List<Certificate> reports = certificateRepo.findAllByDateBetween(dateBegin, dateEnd);
        if (reports == null || reports.size() == 0) {
            bot.execute(new DeleteMessage(chatId, messagePrevReport));
            bot.execute(new SendMessage(chatId, "qwerty"));
            return;
        }
        ...
        //  ------------------------------------------------------------- //
        int rowIndex = 0;
        Message message = messageRepo.findByIdAndLangId(Const.TABLE_CERTIFICATE_MESSAGE, LanguageService.getLanguage(chatId).getId());
        ...
        List<List<String>> info = reports.stream().map(x -> {
            ...
            list.add(DateUtil.getDayDate(x.getCourse_end_date()));
            list.add(x.getFull_name_as_certificate());
            list.add(x.getCertificate_request());
            return list;
    ...
    private void sendFile(long chatId, DefaultAbsSender bot, Date dateBegin, Date dateEnd, int messagePrevReport) throws IOException, TelegramApiException {
        String fileName = "Запросы по сертификату за: " + DateUtil.getDayDate(dateBegin) + " - " + DateUtil.getDayDate(dateEnd) + ".xlsx";
