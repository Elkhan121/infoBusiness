...
import com.example.czn.util.type.ParseMode;
...
//    private static DaoFactory daoFactory = DaoFactory.getFactory();
    ...
    public int sendMessage(long messageId, long chatId, Contact contact, String photo) throws TelegramApiException {
        int result = 0;
        language = LanguageService.getLanguage(chatId);
        Message message = messageRepo.findByIdAndLangId(messageId,language.getId());//daoFactory.getMessageDao().getMessage(messageId);
        SendMessage sendMessage = new SendMessage();
        ...
        if (message.getKeyboardId() != null && keyboardMarkUpRepo.findById(message.getKeyboardId()).isPresent()) {
            sendMessage.setReplyMarkup(keyboardMarkUpRepo.findById(message.getKeyboardId().intValue()));
        }
